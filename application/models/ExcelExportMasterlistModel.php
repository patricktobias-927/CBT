...
$query = $this->db->query("SELECT s.student_id as student_id, s.first_name as first_name, s.middle_name as middle_name, s.last_name as last_name, s.gender as gender, CONCAT(SUBSTRING(ad.school_year, 3, 2), SUBSTRING(ad.school_year, 8, 2), '-', RIGHT(s.school_code, 4), '-', RIGHT(s.grade_level, 2), '-', RIGHT(s.section_code, 4), '-', RIGHT(s.LRN, 2)) as concated, 

CONCAT(mi.school_acronym, '_', RIGHT(s.grade_level, 2), s.section_code, '_', s.student_id) as username,
CONCAT(s.section_code, RIGHT(s.student_id, 2), '_', s.last_name) as pass_word,
