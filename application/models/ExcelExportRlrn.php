<?php
class ExcelExportRlrn extends CI_Model
{
 function fetch_data()
 {
$sid = $this->input->post('select_lrn2');
...
FROM `cbt_students` s LEFT JOIN cbt_add_section ad ON (s.section_id = ad.section_id) WHERE s.student_id = '".$sid."' ORDER BY s.LRN;");
